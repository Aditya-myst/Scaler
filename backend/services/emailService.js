...
async function sendOrderConfirmationEmail(userEmail, userName, order) {
    const itemsHtml = order.OrderItems?.map(item => `
        <tr>
            <td style="padding:10px 8px;border-bottom:1px solid #eee;font-size:14px">${item.Product?.name || 'Product'}</td>
            <td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:center;font-size:14px">${item.quantity}</td>
            <td style="padding:10px 8px;border-bottom:1px solid #eee;text-align:right;font-size:14px">₹${fmt((item.price || 0) * (item.quantity || 0))}</td>
        </tr>
    ...
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#f9f9f9;padding:20px">
        ...
        <div style="background:white;padding:30px;border-radius:0 0 8px 8px;box-shadow:0 2px 8px rgba(0,0,0,0.1)">
            ...
            <table style="width:100%;border-collapse:collapse;margin-bottom:20px">
                <thead>
                    <tr style="background:#f8f9fa">
                        <th style="padding:10px 8px;text-align:left;font-size:12px;color:#666;text-transform:uppercase">Item</th>
                        <th style="padding:10px 8px;text-align:center;font-size:12px;color:#666;text-transform:uppercase">Qty</th>
                        <th style="padding:10px 8px;text-align:right;font-size:12px;color:#666;text-transform:uppercase">Price</th>
                    </tr>
                </thead>
            ...
            <div style="background:#fff8e1;border:1px solid #ffe082;border-radius:8px;padding:16px;margin-top:20px">
                <p style="margin:0 0 6px;font-size:13px;color:#856404">📦 <strong>Estimated Delivery:</strong> 3-7 business days</p>
                <p style="margin:0;font-size:13px;color:#856404">🚚 <strong>Shipping to:</strong> ${order.shipping_address}</p>
            </div>
    ...
    await resend.emails.send({
        from: 'Amazon.in <onboarding@resend.dev>',  // use this until you verify a domain
        to: userEmail,
